...
export class UsuarioService {
    ...
    registrarCliente(data: ClienteRegister): Observable<any> {
        ...
        return this.http.post<ClienteRegister>(`${this.apiUrl}/cliente`, payload);
    }

    registrarBarbero(data: BarberoRegister): Observable<any> {
        return this.http.post<BarberoRegister>(`${this.apiUrl}/barbero`, data);
    }

    registrarAdmin(data: AdminRegister): Observable<any> {
        const payload = { ...data, idRol: 1 } as AdminRegister;
    ...
    resetPassword(idUsuario: number, data: ResetPasswordRequest): Observable<any> {
        return this.http.put<any>(`${this.apiUrl}/${idUsuario}/reset-password`, data);
    }

    updateUsername(idUsuario: number, data: UpdateUsernameRequest): Observable<any> {
        return this.http.patch<any>(`${this.apiUrl}/${idUsuario}/username-update`, data);
    }
    ...
    regenerarQr(idUsuario: number): Observable<Blob> {
        return this.http.post(`${this.apiUrl}/${idUsuario}/qr/regenerar`, {}, { responseType: 'blob' });
    }

    asignarPin(idUsuario: number, pin: string): Observable<any> {
        return this.http.patch<any>(`${this.apiUrl}/${idUsuario}/pin`, { pin }
        );
    }
    ...
    filtrar(filtros: { rol?: string; tieneQr?: boolean; multiplesRoles?: boolean; }, page: number = 0, size: number = 10): Observable<ApiResponse<Page<UsuarioTablaResponse>>> {
        ...
        if (filtros.tieneQr !== undefined) {
            params = params.set('tieneQr', filtros.tieneQr);
        }

        if (filtros.multiplesRoles !== undefined) {
            params = params.set('multiplesRoles', filtros.multiplesRoles);
        }

        return this.http.get<ApiResponse<Page<UsuarioTablaResponse>>>(`${this.apiUrl}/filtrar`, { params });
    }

    buscar(texto: string, page: number = 0, size: number = 10): Observable<ApiResponse<Page<UsuarioTablaResponse>>> {
        const params = new HttpParams().set('texto', texto).set('page', page).set('size', size);
        return this.http.get<ApiResponse<Page<UsuarioTablaResponse>>>(`${this.apiUrl}/buscar`, { params });
    }


    asignarRoles(idUsuario: number, data: AssignRolesRequest): Observable<any> {
        return this.http.patch<any>(`${this.apiUrl}/${idUsuario}/roles`, data);
    }

    quitarRol(idUsuario: number, idRol: number): Observable<any> {
        return this.http.delete<any>(`${this.apiUrl}/${idUsuario}/roles/${idRol}`);
    }
    ...
    obtenerRolesUsuario(idUsuario: number): Observable<ApiResponse<Rol[]>> {
        return this.http.get<ApiResponse<Rol[]>>(`${this.apiUrl}/${idUsuario}/roles`);
    }

    obtenerPermisosUsuario(idUsuario: number, page: number = 0, size: number = 10): Observable<ApiResponse<Page<Permiso>>> {
        const params = new HttpParams().set('page', page).set('size', size);
        return this.http.get<ApiResponse<Page<Permiso>>>(`${this.apiUrl}/${idUsuario}/permisos`, { params });
    }
    changeMyPassword(data: ChangePasswordRequest): Observable<any> {
        return this.http.patch<any>(`${this.apiUrl}/me/change-password`, data);
    }
}